...
def generate_advice(student_id):
    student = students.get(student_id) #get into inner dictionary
    if not student:
        return jsonify({"error": "Student not found"}), 404 
    major = student.get("major")
    if not major: 
        return jsonify({"error": "Student major is required to generate advice"}), 400
    try: #if there is error raised, it will go to the exception field
    ...
    return jsonify({"id": student['id'], "major": major, "advice": student["advice"]}), 200 
